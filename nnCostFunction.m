...
for i=1:m
  for j=1:num_labels
    w=w+f(i,j)*log(p(i,j))+(1-f(i,j))*log(1-p(i,j));
  end
end
J=w*(-1)/m; 
%l1=0;
%l2=0;
J=J+lambda*(sum(sum(Theta1(:,2:size(Theta1,2)).*Theta1(:,2:size(Theta1,2))))+sum(sum(Theta2(:,2:size(Theta2,2)).*Theta2(:,2:size(Theta2,2)))))/2/m;
      
  for i=1:m
    for j=1:num_labels
      delta3(i,j)=p(i,j)-f(i,j);
    end
  end
